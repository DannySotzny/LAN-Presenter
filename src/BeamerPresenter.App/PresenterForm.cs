...
internal sealed class PresenterForm : Form
{
    ...
    public PresenterForm(WebApplication host)
    {
        ...
        Text = "Beamer Presenter for LAN-Parties"; MinimumSize = new Size(610, 370); StartPosition = FormStartPosition.CenterScreen;
    ...
    private Control CreateContent()
    {
        ...
        AddRow(root, 0, "Web UI:", _webUrl); AddRow(root, 1, "Videoordner:", _mediaFolder); AddRow(root, 2, "Web-Passwort:", _password); AddRow(root, 3, "Passwort wiederholen:", _passwordRepeat); AddRow(root, 4, "Schutzstatus:", _passwordStatus);
        var save = new Button { Text = "Einstellungen speichern", AutoSize = true, Anchor = AnchorStyles.Left }; save.Click += async (_, _) => await SaveSettingsAsync(); root.Controls.Add(save, 1, 5);
        root.Controls.Add(new Label { AutoSize = true, Text = "Port-Änderungen gelten nach einem Neustart." }, 1, 6); return root;
    }
    ...
    private async Task LoadSettingsAsync() { var settings = await _settingsService.GetAsync(); _mediaFolder.Text = settings.MediaFolder; _webUrl.Text = $"http://localhost:{settings.WebPort}"; _passwordStatus.Text = string.IsNullOrWhiteSpace(settings.PasswordHash) ? "Noch nicht eingerichtet" : "Aktiv"; }
    private async Task SaveSettingsAsync()
    {
        ...
        var settings = await _settingsService.GetAsync(); settings.MediaFolder = _mediaFolder.Text.Trim(); await _settingsService.SaveAsync(settings); if (!string.IsNullOrWhiteSpace(_password.Text)) await _settingsService.SetWebPasswordAsync(_password.Text);
