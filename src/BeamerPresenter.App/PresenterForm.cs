...
internal sealed class PresenterForm : Form
{
    ...
    public PresenterForm(WebApplication host)
    {
        ...
        Text = "Beamer Presenter for LAN-Parties"; MinimumSize = new Size(610, 410); StartPosition = FormStartPosition.CenterScreen;
    ...
    private Control CreateContent()
    {
        ...
        AddRow(root, 0, "Web UI:", _webUrl); AddRow(root, 1, "Web UI Port:", _webPort); AddRow(root, 2, "Videoordner:", _mediaFolder); AddRow(root, 3, "Web-Passwort:", _password); AddRow(root, 4, "Passwort wiederholen:", _passwordRepeat); AddRow(root, 5, "Schutzstatus:", _passwordStatus);
        var save = new Button { Text = "Einstellungen speichern", AutoSize = true, Anchor = AnchorStyles.Left }; save.Click += async (_, _) => await SaveSettingsAsync(); root.Controls.Add(save, 1, 6);
        root.Controls.Add(new Label { AutoSize = true, Text = "Port-Änderungen gelten nach einem Neustart." }, 1, 7); return root;
    }
